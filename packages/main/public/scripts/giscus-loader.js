...
(function () {
  ...
  var repo = (wrapper && wrapper.getAttribute('data-repo')) || container.getAttribute('data-repo') || ''
  ...
  if (!repo || repo.indexOf('/') === -1 || repo.indexOf('REPLACE_WITH') !== -1 || !repoId || repoId.indexOf('REPLACE_WITH') !== -1) {
    var p = document.createElement('p')
    p.className = 'text-xs text-volf-muted-2 mt-2'
    p.textContent = 'Comentarios desactivados hasta configurar Giscus (repo + Discussions). Ver src/components/Giscus.astro.'
