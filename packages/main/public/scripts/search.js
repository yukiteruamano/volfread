...
(function () {
  ...
  function init() {
    ...
    function setOpen(open) {
      ...
      panel.hidden = !open;
      if (open) setTimeout(function () { input.focus(); }, 10);
    }
    ...
    function render(list) {
      ...
      list.slice(0, 5).forEach(function (p) {
        ...
        a.href = p.url;
        a.className = 'flex gap-3 rounded-xl border border-transparent bg-volf-bg/60 p-2 hover:border-volf-orange/30 hover:bg-volf-surface transition';
        var imgWrap = document.createElement('div');
        imgWrap.className = 'shrink-0 w-14 h-14 rounded-lg overflow-hidden bg-volf-surface border border-volf-border';
        if (p.cover) {
        ...
        a.appendChild(imgWrap); a.appendChild(txt);
        li.appendChild(a);
    ...
    input.addEventListener('input', function () {
      ...
      debounce = setTimeout(function () {
        ...
        var nq = norm(q);
        if (!nq) { render([]); filterCards(''); return; }
        var filtered = idx.filter(function (p) {
          var hay = norm(p.title + ' ' + p.description + ' ' + (p.tags||[]).join(' ') + ' ' + (p.categories||[]).join(' '));
    ...
    if (results) results.addEventListener('click', function (e) { if (e.target.closest('a')) setOpen(false); });
  }
  ...
  function initWithCleanup() {
    ...
    init();
    document.addEventListener('astro:before-swap', function () { if (ctrl) ctrl.abort(); }, { signal: ctrl.signal });
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initWithCleanup);
