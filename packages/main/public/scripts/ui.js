...
(function () {
    ...
    document.querySelectorAll('.copy-btn').forEach(function (btn) {
      ...
      btn.addEventListener('click', function () {
        ...
        navigator.clipboard.writeText(code).then(function () {
          ...
          btn.textContent = btn.getAttribute('data-copied-text') || (document.documentElement.lang === 'en' ? 'Copied ✓' : 'Copiado ✓');
          var t = setTimeout(function () { btn.innerHTML = origHTML; }, 2000);
          // limpia timeout si navega antes
          document.addEventListener('astro:before-swap', function () { clearTimeout(t); }, { once: true, signal: ctrl ? ctrl.signal : undefined });
        });
      });
    });
  }
  function initMenu() {
    ...
    function setOpen(open) {
      ...
      menu.hidden = !open;
      var m = btn.querySelector('.menu-icon'), c = btn.querySelector('.close-icon');
      if (m) m.classList.toggle('hidden', open);
      if (c) c.classList.toggle('hidden', !open);
      if (open) { var a = menu.querySelector('a'); a && a.focus(); }
    }
    ...
    if (!document.documentElement.dataset.uiGlobalBound) {
      ...
      document.addEventListener('click', function (e) { if (menu && !menu.hidden && !e.target.closest('#mobile-nav-root')) setOpen(false); });
    }
  }
  function init() {
    if (ctrl) ctrl.abort();
